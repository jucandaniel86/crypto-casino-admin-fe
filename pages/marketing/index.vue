...
// Small KPI card local component (minimal)
const KpiCard = defineComponent({
  props: { title: String, value: [String, Number], suffix: String },
  template: `
    <v-card rounded="0" variant="flat" class="kpi-card">
      <v-card-text>
        <div class="text-caption text-medium-emphasis">{{ title }}</div>
        <div class="text-h5 font-weight-bold">
          {{ value }} <span v-if="suffix" class="text-caption">{{ suffix }}</span>
        </div>
      </v-card-text>
    </v-card>
  `,
        ...
        <template v-if="tab === 'overview'">
          <v-row>
            <v-col cols="12" md="3">
              <KpiCard
                title="Registrations"
                :value="overview?.cards?.registrations ?? 0"
              />
            </v-col>
            <v-col cols="12" md="3">
              <KpiCard title="FTD" :value="overview?.cards?.ftd ?? 0" />
            </v-col>
            <v-col cols="12" md="3">
              <KpiCard
                title="Active players"
                :value="overview?.cards?.active_players ?? 0"
              />
            </v-col>
            <v-col cols="12" md="3">
              <KpiCard
                title="GGR"
                :value="overview?.cards?.ggr ?? '0'"
                suffix="PEP"
              />
            </v-col>
          </v-row>
