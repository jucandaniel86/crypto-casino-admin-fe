	...
	<div>
		<v-data-table-server :headers="activeHeaders" :items="items" :options.sync="options" :loading="loading"
						...
						<v-row no-gutters>
							<v-col md="5" cols="12">
								<v-breadcrumbs :items="[
									...
									{ title: 'Transactions', href: '#', disabled: true },
			...
			<template v-slot:item="{ item }">
				<tr>
					<td v-if="checkActiveData('round_id')">{{ item.round_id }}</td>
					<td v-if="checkActiveData('operator_player_id')">{{ item.operator_player_id }}</td>
					<td v-if="checkActiveData('operator_session_id')">{{ item.operator_session_id }}</td>
					...
					<td v-if="checkActiveData('balance')">{{ convertNumber(item.balance) }}</td>
					<td v-if="checkActiveData('currency')">{{ item.currency }}</td>
					<td v-if="checkActiveData('currency')">{{ formatDate(item.timestamp) }}</td>
				</tr>
			</template>
		</v-data-table-server>
	</div>
