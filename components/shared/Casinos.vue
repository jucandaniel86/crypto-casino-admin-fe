...
type Casino = {
  ...
  name: string;
  logo_absolute_path?: string | null;
};
  ...
  >
    <template #item="{ props: itemProps, item }">
      <v-list-item v-bind="itemProps">
        <template #prepend>
          <v-avatar size="22" v-if="item?.raw?.logo_absolute_path">
            <v-img :src="item.raw.logo_absolute_path" alt="" />
          </v-avatar>
        </template>
      </v-list-item>
    </template>
    <template #selection="{ item }">
      <div class="d-flex align-center ga-2">
        <v-avatar size="18" v-if="item?.raw?.logo_absolute_path">
          <v-img :src="item.raw.logo_absolute_path" alt="" />
        </v-avatar>
